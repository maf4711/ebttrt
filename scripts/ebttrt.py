#!/usr/bin/env python3
"""ebttrt CLI — even better than the real thing."""
...
from ebttrt_lib import (  # noqa: F401 — re-export for tests
    LOOP,
    MAX_CONTEXT_CHARS,
    VERSION,
    cmd_improve,
...
from ebttrt_activate import cmd_activate
from ebttrt_review import cmd_review, review_blocker
from ebttrt_prove import (
    DEFAULT_TIMEOUT,
    cmd_consult,
    cmd_prove,
    cmd_receipt_check,
...
def cmd_status() -> int:
    home = ensure_dirs()
    root = plugin_root()
    rule = grok_home() / "rules" / "ebttrt-loop.md"
    plugin_link = grok_home() / "plugins" / "ebttrt"
    cwd = Path.cwd()
    print(f"ebttrt {VERSION}")
    print(f"plugin:   {root}")
    print(f"data:     {home}")
...
def cmd_doctor() -> int:
    ...
    for rel in (
        "plugin.json",
        "skills/ebttrt/SKILL.md",
        "hooks/hooks.json",
        "rules/ebttrt-loop.md",
        "scripts/ebttrt.py",
        "scripts/ebttrt_loop.py",
        "scripts/ebttrt_prove.py",
        "scripts/ebttrt_activate.py",
        "scripts/ebttrt_review.py",
        "skills/ebttrt-activate/SKILL.md",
    ):
    ...
    link = grok_home() / "plugins" / "ebttrt"
    if not link.exists():
        issues.append(f"plugin not linked at {link}")
    cli = grok_home() / "bin" / "ebttrt"
    if not cli.exists():
    ...
    if issues:
        ...
        for item in issues:
            print(f"  - {item}")
        print("fix:  ebttrt activate")
        return 1
    print("ebttrt doctor: OK")
    print(f"  plugin {root}")
...
def cmd_context() -> int:
    ...
    if not path.exists():
        print("(no ebttrt context yet — start a Grok session or run ebttrt begin)")
        return 0
...
def cmd_done(evidence: str, cwd: Path) -> int:
    ...
    if not last or not last.get("ok") or not prove_is_fresh(cwd, last):
        print("need a fresh passing `ebttrt prove`", file=sys.stderr)
        return 1
...
def append_enabled_plugin() -> None:
    ...
    text = re.sub(r'(?m)^\s*"ebttrl",?\s*\n', "", text)
    if re.search(r'(?m)^\s*"ebttrt"\s*,?\s*$', text):
...
def cmd_install() -> int:
    ...
    if cli.is_symlink() or cli.exists():
        cli.unlink()
    cli.symlink_to(root / "scripts" / "ebttrt.py")
    cli.chmod(cli.stat().st_mode | stat.S_IXUSR)
    alias = home / "bin" / "ebttrl"
    if alias.is_symlink() or alias.is_file():
        alias.unlink()
    alias.symlink_to(root / "scripts" / "ebttrt.py")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name, help_ in (
        ...
        ("eval", "run the ebttrt unit suite"),
    ):
