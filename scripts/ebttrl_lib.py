"""ebttrl core: paths, shield, receipts, hooks. Stdlib only."""
...
def data_home() -> Path:
    override = os.environ.get("EBTTRL_HOME")
    return Path(override).expanduser() if override else grok_home() / "ebttrl"
...
def deny_reason(command: str) -> str | None:
    ...
    for pat in DENY_COMMANDS:
        if pat.search(command):
            return f"ebttrl shield blocked a dangerous command ({pat.pattern})"
    return None


def hook_session_start(event: dict[str, Any]) -> int:
    from ebttrl_loop import emit_hook_context, inject_session_context, write_session_context
...
def hook_posttool(event: dict[str, Any]) -> int:
    from ebttrl_loop import note_edit

    return note_edit(event)


def hook_stop(event: dict[str, Any]) -> int:
    from ebttrl_loop import hook_stop as _stop
...
def receipt_matches(cwd: Path) -> dict[str, Any] | None:
    from ebttrl_loop import receipts_for
...
def cmd_remember(
    ...
    if not force:
        if receipt_matches(here) is None:
            print("need a MATCH receipt for this workspace (ebttrl done) or --force", file=sys.stderr)
            return 1
    home = ensure_dirs()
...
def cmd_improve(cwd: Path) -> int:
    from ebttrl_loop import load_receipts
...
def cmd_shield(root: Path) -> int:
    ...
    print(f"ebttrl shield  {letter}  {len(findings)} finding(s)  {root}")
