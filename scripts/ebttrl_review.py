...
from ebttrl_loop import journal, workspace_dir
...
def review_blocker(cwd: Path) -> str | None:
    ...
    if rec is None:
        return "review required (multi-file or public API) — ebttrl review"
    if not review_fresh(cwd, rec):
        return "review drifted (tree changed) — re-run ebttrl review"
