...
def load_config(cwd: Path) -> dict[str, Any]:
    for name in (".ebttrt.json", "ebttrt.json", ".ebttrl.json", "ebttrl.json"):
        path = cwd / name
        if not path.is_file():
            continue
        try:
...
def discover_prove(cwd: Path) -> tuple[list[list[str]], str]:
    ...
    if cfg.get("prove"):
        try:
            return parse_prove_commands(cfg["prove"]), ".ebttrt.json"
...
def cmd_receipt_check(cwd: Path) -> int:
    from ebttrt_loop import receipts_for, workspace_slug
