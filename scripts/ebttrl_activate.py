"""Activate ebttrl on this Mac. No hardcoded user home paths."""
...
from ebttrl_lib import grok_home, plugin_root
...
def looks_like_ebttrl(path: Path) -> bool:
    return (path / "plugin.json").is_file() and (path / "scripts" / "ebttrl.py").is_file()
...
def find_ebttrl() -> Path | None:
    env = os.environ.get("EBTTRL_ROOT")
    if env is not None:
        p = Path(env).expanduser()
        return p if looks_like_ebttrl(p) else None
    root = plugin_root()
    if looks_like_ebttrl(root):
        return root
    here = Path(__file__).resolve().parent.parent
    if looks_like_ebttrl(here):
        return here
    cwd = Path.cwd()
    for cand in (cwd, *cwd.parents):
        if looks_like_ebttrl(cand):
            return cand
    home_dev = Path.home() / "Developer" / "ebttrl"
    if looks_like_ebttrl(home_dev):
        return home_dev
    return None
...
def zshrc_path() -> Path:
    env = os.environ.get("EBTTRL_ZSHRC")
    if env:
...
def ensure_zsh_path() -> str:
    ...
    override = os.environ.get("EBTTRL_ZSHRC")
...
def install_user_skill(repo: Path) -> Path:
    src = repo / "skills" / "ebttrl-activate" / "SKILL.md"
    dest_dir = grok_home() / "skills" / "ebttrl-activate"
...
def wire_hoheit(hoheit: Path) -> list[str]:
    ...
    ebt = hoheit / ".ebttrl.json"
    if not ebt.is_file():
        ebt.write_text('{\n  "prove": "scripts/hoheit-prove"\n}\n', encoding="utf-8")
        notes.append("wrote .ebttrl.json")
...
def cmd_activate() -> int:
    repo = find_ebttrl()
    if repo is None:
        print("ebttrl repo not found on this Mac.", file=sys.stderr)
        print("clone https://github.com/maf4711/ebttrl.git to ~/Developer/ebttrl, then:", file=sys.stderr)
        print("  python3 ~/Developer/ebttrl/scripts/ebttrl.py activate", file=sys.stderr)
        return 2
    os.environ["GROK_PLUGIN_ROOT"] = str(repo)
    from ebttrl import cmd_install

    chmod_exec(repo / "scripts" / "activate.sh")
    chmod_exec(repo / "scripts" / "install.sh")
    chmod_exec(repo / "scripts" / "ebttrl.py")
    print(f"repo:    {repo}")
    rc = cmd_install()
    ...
    print()
    print("next: new Grok session  ·  /plugins → r  ·  ebttrl doctor")
    return rc
