"""ebttrt loop state: begin → phase → next → done. One active loop per workspace."""
...
PHASE_SKILL = {
    ...
    "test": "ebttrt-tdd",
    "implement": "ebttrt",
    "review": "ebttrt-review",
    "verify": "ebttrt-verify",
    "remember": "ebttrt-remember",
    "improve": "ebttrt-improve",
}
...
def cmd_next(cwd: Path) -> int:
    ...
    print(f"run:     ebttrt phase {nxt[0]}")
    return 0
...
def format_loop(active: dict[str, Any]) -> str:
    ...
    skill = PHASE_SKILL.get(phase, "ebttrt")
    lines = [
...
def inject_session_context(event: dict[str, Any]) -> str:
    ...
    last_prove = None
    try:
        from ebttrt_prove import load_last_prove
    ...
    lines = [
        ...
        f"prove: {prove}",
        f"receipt: {receipt}",
        "next: ebttrt prove --record && ebttrt done",
    ]
    text = "\n".join(lines) + "\n"
...
def write_session_context(event: dict[str, Any]) -> str:
    ...
    if active:
        lines.extend(
            [
                ...
                f"- phase: {active.get('phase')} → skill `{PHASE_SKILL.get(str(active.get('phase')), 'ebttrt')}`",
                f"- verified: {'yes' if active.get('verified') else 'no'}",
    ...
    if last_prove:
        ...
        lines.append("(none — `ebttrt prove`)")
